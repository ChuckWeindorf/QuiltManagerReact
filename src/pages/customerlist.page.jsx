   ...
   useEffect(() => {retrieveCustomers()}, []);
            ...
            function ShowCusts()
            {
             ...
             {arrCust.map((cust, index) => {
                 return(<tr className="custOne" key={`c${index}`}>
                       ...
                       <td key={`td11${index}`}>
                        <NavLink className="navType" to="/customerpage" 
                                 state={{navState: `${cust.CustomerID}`}}><button type="button" className="custButton">Cust</button></NavLink></td>
                       </tr>
                       );
                    })}
             </>)
...
//<td key={`td6${index}`}>{cust.HomePhone}</td>
   ...
   <table id="allCust">
    <thead>
     ...
     <th>St</th>
     <th>Zip</th>
     <th><NavLink className="navType" to="/customerpage" 
          state={{navState: "New"}}><button type="button" className="custButton">NEW</button></NavLink>
     </th>
     </tr>
    </thead>
    <tbody>
