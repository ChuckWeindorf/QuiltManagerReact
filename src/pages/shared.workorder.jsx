...
function GetBackground(work){
  ...
  function canShowStatus(work, cstrStatus) {
    ...
    else if (work.CompleteDate) {vbolShow = false}
    else if (cstrStatus == "Warning" && 
...
return (
<>
 {arrWorks.map((work, index) => {if (canShowStatus(work, cstrStatus)) {
     return(
           <div className="workOne" key={`d${index}`} 
           ...
           Customer: {work.CustomerID}<br/>
           Type: {work.Worktype}<br/>
           Description: {work.WorkDescription}<br/>
           Order Date: {simpleDate(work.OrderDate,false)}<br/>
           Receive Date: {simpleDate(work.ReceivedDate,false)}<br/>
           Start Date: {simpleDate(work.StartDate,false)}<br/>
           Complete Date: {simpleDate(work.CompleteDate,false)}<br/>
           Target Date: {simpleDate(work.TargetDate,false)}<br/>
           Paid: {work.Paid ? "Yes": "No"}<br/>
           QuoteAmount: {work.QuoteAmount}
           </div>
           <NavLink className="navType" to="/workorder" state={{navState: `${work.SaleID}`}}>
                  <button type="button" className="goWorkButton">SEE</button>
                  </NavLink>
           </div>
           )};
        })}
